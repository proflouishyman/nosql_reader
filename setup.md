...
1. If `.env` does not already exist (the bootstrap script creates it when missing), copy the template:
   ```bash
   cp .env.example .env
   ```
...
1. If `.env` was not generated by the bootstrap script, copy the template:
   ```bash
   cp .env.example .env
   ```
