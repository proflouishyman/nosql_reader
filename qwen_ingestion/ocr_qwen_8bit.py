...
# Purpose: Batch OCR with Qwen/Qwen3-VL-30B-A3B-Instruct-FP8 across multiple GPUs,
...
# Force spawn method for multiprocessing with CUDA
...
PROCESSES_PER_GPU = 1        # 1 process per GPU for stability with FP8
...
def worker_loop(worker_id: int,
    ...
    try:
        ...
        # Use memory-efficient loading with specific device mapping
        model = Qwen3VLForConditionalGeneration.from_pretrained(
            MODEL_NAME,
            torch_dtype=torch.float16,
            device_map={"": f"cuda:{gpu_slot}"},  # Explicitly map to specific GPU
        )
        t1 = time.time()
