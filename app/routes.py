...
def load_exploration_notebook():
    """Load a saved notebook/report and set it as the active exploration report."""
    global _last_exploration_report
    ...
    try:
        ...
        is_full_report = (
            isinstance(loaded_payload, dict)
            and 'exploration_metadata' in loaded_payload
            and 'question_synthesis' in loaded_payload
            and isinstance(loaded_payload.get('corpus_map'), dict)
            and 'statistics' in loaded_payload.get('corpus_map', {})
        )
        if isinstance(loaded_payload, dict) and not is_full_report:
            _last_exploration_report = {
                'corpus_map': {
                    'statistics': loaded_payload.get('corpus_map', {}),
                    'archive_notes': 'Loaded notebook snapshot from disk. Run a new exploration for refreshed synthesis outputs.',
                },
                'questions': _as_list(loaded_payload.get('questions')),
                'patterns': _as_list(loaded_payload.get('patterns')),
                'contradictions': _as_list(loaded_payload.get('contradictions')),
                'entities': _as_list(loaded_payload.get('entities')),
                'question_synthesis': {
                    'themes': [],
                    'gaps': [],
                },
...
def _append_run_event(run: Dict[str, Any], message: str) -> None:
    """Append one progress event entry to a run buffer."""
