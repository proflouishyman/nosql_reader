(function() {
    ...
    ready(function() {
        ...
        const mountTextarea = document.getElementById('mountDirectories'); // change: Capture the manual mount list textarea for dynamic Docker updates.
        const mountStatus = document.querySelector('[data-mount-status]'); // change: Surface mount update feedback without relying solely on alert dialogs.
        ...
        function updateMountStatus(message, type) {
            // change: Provide inline confirmation or error feedback for mount updates.
            if (!mountStatus) {
                if (message) {
            ...
            mountStatus.hidden = !message;
            mountStatus.dataset.statusType = type || 'info';
        }

        async function submitMountsHandler(event) {
            // change: Forward host directory selections to the backend mount updater.
            if (event) {
                event.preventDefault();
            }
            if (!mountTextarea) {
                updateMountStatus('Mount directory input is unavailable on this page.', 'error');
                return;
            }
            const directories = parseDirectoriesFromInput(mountTextarea.value);
            if (!directories.length) {
                updateMountStatus('Add at least one directory before updating mounts.', 'error');
                return;
            }
            updateMountStatus('Submitting mount update…', 'info');
            try {
                ...
                if (!response.ok) {
                    const message = payload && payload.error ? payload.error : `Mount update failed with status ${response.status}`;
                    updateMountStatus(message, 'error');
                    window.alert(message);
                    return;
                }
                const message = payload && payload.message ? payload.message : 'Mounts updated. Docker is rebuilding…';
                updateMountStatus(message, 'success');
                window.alert(message);
            } catch (error) {
                updateMountStatus(`Failed to submit mount update: ${error.message}`, 'error');
                window.alert(`Failed to submit mount update: ${error.message}`);
            }
        }

        window.submitMounts = submitMountsHandler; // change: Allow the template button to trigger the asynchronous mount update.
