...
(function () {
            ...
            btn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (loadData.status === 'loaded') {
                        ...
                        activeReport = report;
                        window._ceLastReport = report; // Keep export payload aligned after notebook loads.
                        setState('results');
    ...
    if (exportMemoBtn) exportMemoBtn.addEventListener('click', () => exportResearchMemo(activeReport));
    ...
    function exportResearchMemo(report) {
        if (!report) return;

        const now = new Date();
        const dateStr = now.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
        const meta = report.exploration_metadata || {};
        const synth = report.question_synthesis || {};
        const gaps = synth.gaps || [];
        const themes = synth.themes || [];

        let memo = 'RESEARCH MEMO\n';
        memo += `${'='.repeat(60)}\n\n`;
        memo += `Date: ${dateStr}\n`;
        memo += 'Collection: Baltimore & Ohio Railroad, Relief Department Records\n';
        memo += `Documents sampled: ${meta.documents_read || '—'}\n`;
        memo += `Reading strategy: ${meta.strategy || '—'}\n`;
        if (meta.research_lens) {
            memo += `Research lens: ${Array.isArray(meta.research_lens) ? meta.research_lens.join(', ') : meta.research_lens}\n`;
        }
        memo += '\n';

        if (synth.grand_narrative) {
            memo += 'CENTRAL RESEARCH QUESTION\n';
            memo += `${'-'.repeat(40)}\n`;
            memo += `${synth.grand_narrative}\n\n`;
        }

        if (themes.length) {
            memo += 'RESEARCH AGENDA\n';
            memo += `${'-'.repeat(40)}\n`;
            themes.forEach((theme, ti) => {
                memo += `\n${ti + 1}. ${theme.theme || theme.title || theme.name || 'Theme'}\n`;
                const questions = theme.questions || [];
                questions.forEach((q, qi) => {
                    const qText = typeof q === 'string' ? q : q.question_text || q.text || q.question || '';
                    memo += `   ${qi + 1}.${qi + 1 < 10 ? ' ' : ''}${qText}\n`;
                });
            });
            memo += '\n';
        }
        ...
        if (gaps.length) {
            ...
            memo += `${'-'.repeat(40)}\n`;
            gaps.forEach((gap) => {
                const gText = typeof gap === 'string' ? gap : gap.description || gap.text || gap.gap || '';
                memo += `- ${gText}\n`;
            });
            memo += '\n';
        }

        memo += 'METHODOLOGY NOTE\n';
        memo += `${'-'.repeat(40)}\n`;
        memo += 'This memo was generated by systematic AI-assisted reading of the corpus. ';
        memo += `The Explorer sampled approximately ${meta.documents_read || 'N'} documents `;
        memo += `using the "${meta.strategy || 'balanced'}" strategy. `;
        memo += 'Themes and questions were generated by language model synthesis and should ';
        memo += 'be treated as research hypotheses requiring archival verification.\n';

        const blob = new Blob([memo], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `research-memo-${now.toISOString().slice(0, 10)}.txt`;
        a.click();
        URL.revokeObjectURL(url);
    }
    ...
    fetch('/api/rag/exploration_report')
        ...
        .then((report) => {
            if (report && !report.error) {
                activeReport = report;
                window._ceLastReport = report; // Preserve same report contract for memo export on page reload.
                renderResults(report);
                setState('results');
            }
        })
