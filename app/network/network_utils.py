...
def _filter_edges_by_person_mention_threshold(
    db,
    edges: List[Dict[str, Any]],
    person_min_mentions: int,
) -> List[Dict[str, Any]]:
    """
    Keep only edges whose PERSON endpoints meet a minimum mention_count threshold.

    For each edge:
    - If an endpoint type is PERSON, that endpoint's mention_count must be >= threshold.
    - Non-PERSON endpoints are not constrained by this rule.
    """
    threshold = max(0, int(person_min_mentions or 0))
    if threshold <= 1 or not edges:
        return edges

    person_ids: Set[str] = set()
    for edge in edges:
        if edge.get("source_type") == "PERSON" and edge.get("source_id"):
            person_ids.add(str(edge["source_id"]))
        if edge.get("target_type") == "PERSON" and edge.get("target_id"):
            person_ids.add(str(edge["target_id"]))

    if not person_ids:
        return edges

    person_info = get_entities_bulk(db, list(person_ids))

    def _person_ok(entity_id: str) -> bool:
        mention_count = (person_info.get(str(entity_id)) or {}).get("mention_count", 0)
        return int(mention_count or 0) >= threshold

    filtered: List[Dict[str, Any]] = []
    for edge in edges:
        if edge.get("source_type") == "PERSON" and not _person_ok(str(edge.get("source_id", ""))):
            continue
        if edge.get("target_type") == "PERSON" and not _person_ok(str(edge.get("target_id", ""))):
            continue
...
def get_global_network(
    ...
    raw_edges = list(edges_coll.find(query).sort("weight", -1).limit(limit))
    raw_edges = _filter_edges_by_person_mention_threshold(
        db,
        raw_edges,
    ...
    return {
        ...
        "stats": {
            ...
            "edges_after_person_filter": len(raw_edges),
            "edges_returned": len(edges),
            "nodes_returned": len(nodes),
            "strict_type_filter": strict_type_filter,
            "person_min_mentions": max(0, int(person_min_mentions or 0)),
        },
    }
...
def get_network_documents_for_viewer(
    ...
    edges = _filter_edges_by_person_mention_threshold(
        db,
        edges,
        person_min_mentions=person_min_mentions,
    )
