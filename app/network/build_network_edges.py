...
Usage:
    ...
    python -m network.build_network_edges
    ...
    python -m network.build_network_edges --dry-run
"""
...
# ---------------------------------------------------------------------------
# Allow execution from project root:  python -m network.build_network_edges
# ---------------------------------------------------------------------------
...
def build_entity_lookup(
    ...
    query: Dict[str, Any] = {}
    effective_types = entity_types

    # If no explicit type filter was provided, derive from live data.
    if not effective_types:
        try:
            effective_types = sorted(t for t in linked.distinct("type") if t)
            logger.info("Derived NETWORK_ENTITY_TYPES from linked_entities: %s", effective_types)
        except Exception as exc:
            logger.warning("Failed to derive entity types dynamically: %s", exc)
            effective_types = []

    # Filter by type if configured (empty list after derive means include all)
    if effective_types:
        query["type"] = {"$in": effective_types}
    ...
    logger.info(
        ...
        effective_types or "ALL", max_mention_count,
    )
