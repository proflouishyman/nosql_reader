...
+```
+
+### 2.3 Prepare host directories
+
+Decide where you want MongoDB data, Flask session data, and archive JSON files to live. The defaults in `.env.example` point to subfolders in the repo root.
+
+```bash
+mkdir -p mongo_data
...
+```
+
+You can point these paths elsewhere (for example, under `~/Documents/historical_reader/`) as long as the directories exist before starting Docker.
+
...
+   - `MONGO_ROOT_USERNAME` / `MONGO_ROOT_PASSWORD` – credentials for MongoDB.
...
+
+Create the directories referenced by `.env` using either PowerShell or your WSL shell. Example in PowerShell (run from the project root):
+
+```powershell
+New-Item -ItemType Directory -Force -Path mongo_data, flask_session, archives
+```
+
+Inside WSL, the equivalent is:
+
+```bash
+mkdir -p mongo_data flask_session archives
+```
+
+If you prefer to store data outside the repository (e.g., on `D:\HistoricalReader`), update the paths accordingly when editing `.env`.
+
...
+   cp .env.example .env
+   ```
+   - In PowerShell, use `copy .env.example .env`.
+2. Edit `.env` with your preferred editor:
+   - VS Code: `code .env`
+   - Notepad: `notepad .env`
+   - WSL editor: `nano .env`
+3. Update the same fields described in the macOS section (credentials, host paths, Historian Agent settings). When using Windows paths, either:
...
+```
+
+The web UI will be at [http://localhost:5000](http://localhost:5000). To stop the stack, press `Ctrl+C` or run `docker compose down` in a separate terminal. Container logs appear in your terminal—watch for messages indicating that the Flask app is running and MongoDB has initialised.
+
...
+
+```bash
+docker compose exec app python app/test_db_connection.py
+```
+
+A successful run prints the available databases. If it fails, double-check credentials in `.env` and ensure the `mongodb` service container is healthy (`docker compose ps`).
+
+---
+
+## 5. Next steps
+
+1. Visit [http://localhost:5000](http://localhost:5000) and explore the search UI and Historian Agent.
