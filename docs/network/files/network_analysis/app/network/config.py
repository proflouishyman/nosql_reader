...
class NetworkConfig:
    ...
    def from_env(cls, overrides: Optional[dict] = None) -> "NetworkConfig":
        ...
        config = cls(
            ...
            entity_types=_list(_env("NETWORK_ENTITY_TYPES", "PERSON,ORG,GPE,OCCUPATION")),
            max_mention_count=_int(_env("NETWORK_MAX_MENTION_COUNT", 5000)),
            min_edge_weight=_int(_env("NETWORK_MIN_EDGE_WEIGHT", 2)),
            build_batch_size=_int(_env("NETWORK_BUILD_BATCH_SIZE", 500)),
            default_limit=_int(_env("NETWORK_DEFAULT_LIMIT", 500)),
            default_min_weight=_int(_env("NETWORK_DEFAULT_MIN_WEIGHT", 1)),
            metrics_cache_ttl=_int(_env("NETWORK_METRICS_CACHE_TTL", 86400)),
            max_display_nodes=_int(_env("NETWORK_MAX_DISPLAY_NODES", 500)),
        )
