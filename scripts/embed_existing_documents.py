...
# Added to seed the semantic store with historic document embeddings.  # change rationale comment
...
# Added DocumentChunk import to match chunk dataclass exposed by new module.  # change rationale comment
from app.historian_agent.chunking import DocumentChunk, DocumentChunker
...
class DocumentEmbeddingMigration:
    ...
    def __init__(
        ...
        self.chunker = DocumentChunker(
            ...
            context_fields=["title", "content", "ocr_text", "summary", "description"],  # Added title/description so chunk text mirrors historic keyword coverage.
        )
        
        provider_name = (
            "huggingface" if embedding_provider.lower() in {"local", "huggingface"} else embedding_provider
        )  # Normalized provider name to satisfy EmbeddingService expectations.
    ...
    def _process_batch(self, documents: List[dict]) -> dict:
        ...
        try:
            ...
            # Use chunk_text property to stay aligned with DocumentChunk dataclass.  # change rationale comment
