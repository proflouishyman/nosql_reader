...
class BatchJSONConverter:
    def __init__(self, model="gpt-oss"):  # Using a model we know exists
        self.model = model
        self.check_model()
...
Extract ALL information including dates, names, addresses, amounts, and any other data.
...
"""
    ...
    def process_directory(self, directory, ocr_suffix="_ocr.txt", json_suffix="_structured.json"):
...
def validate_json_files(directory, json_suffix="_structured.json"):
...
if __name__ == "__main__":
    ...
    converter = BatchJSONConverter(model="llama3.1:8b")  # or "gpt-oss:20b" if available
