...
## 2026-03-04 - Add adaptive consultation workflow + researcher-selected reading order (v1.1 addendum)

### Problem
Adaptive explorer needed two missing capabilities from the research consultation addendum:
- Reading order had to be a researcher decision (`archival`, `chronological`, `record_type`, `person`, `balanced`) rather than an implicit strategy default.
- Pre-reading consultation needed a structured, auditable `ResearchBrief` and a one-turn reflection before execution.

Additionally, adaptive runs were starting with empty question graphs when seed extraction returned nothing.

### Solution
- Added `app/historian_agent/research_consultation.py`:
  - `ResearchBrief` dataclass + normalization/compatibility helpers.
  - Consultation reflection builder (`build_reflection`) returning summary, proposed seeds, and recommended sort order.
  - Seed extraction with deterministic fallback (`extract_seed_questions`) so adaptive mode no longer starts with zero seeds.
- Extended adaptive explorer (`app/historian_agent/incremental_explorer.py`):
  - Accepts `research_brief` + `sort_order` inputs.
  - Initializes graph axes from brief axes (run-scoped).
  - Seeds from two passes:
    - prior hypotheses (confidence 0.7)
    - primary lens (confidence 0.5)
  - Uses ordered stratification for adaptive mode and logs missing primary sort fields.
  - Persists `research_brief`, `sort_order`, and missing-sort metrics in exploration metadata.
- Added ordered stratification support (`app/historian_agent/stratification.py`):
  - `build_ordered_strategy(...)` for adaptive traversal.
  - Deterministic sorted reads with null handling controlled by `TIER0_SORT_NULLS` (`last|first|omit`).
  - Tracks missing primary sort field counts.
- Updated API routes (`app/routes.py`):
  - Explorer endpoints accept and forward `research_brief` and `sort_order`.
  - New endpoint: `POST /api/rag/research_consultation/reflect`.
- Updated config (`app/config.py`):
  - Added `TIER0_ADAPTIVE_DEFAULT_SORT` and `TIER0_SORT_NULLS`.
- Updated UI (`app/templates/corpus_explorer.html`, `app/static/corpus_explorer.js`):
  - Added adaptive consultation form fields (Q2–Q5), axis selection, and reading-order choices.
  - Added reflection panel with Confirm/Edit flow before adaptive run launch.
- Added docs:
  - `docs/adaptive_explorer_addendum_v1_1.md`

### Verification
Code quality checks:
- `python -m py_compile app/config.py app/routes.py app/historian_agent/corpus_explorer.py app/historian_agent/stratification.py app/historian_agent/incremental_explorer.py app/historian_agent/research_consultation.py`
- `node --check app/static/corpus_explorer.js`

Tests:
- `PYTHONPATH=app pytest -q app/tests/test_adaptive_question_graph.py app/tests/test_research_consultation.py`
- Result: `9 passed`

Runtime validation (Docker app at `http://localhost:5001`):
- Adaptive 10 (`archival` sort + confirmed `ResearchBrief`):
  - docs=10, batches=1, questions_top=5, graph_nodes=5 (`seed` macros)
- Adaptive 100 (`archival`):
  - docs=100, batches=10, questions_top=5, graph_nodes=5, defrag_snapshots=3
- Legacy 100 (control):
  - docs=100, batches=26, questions_top=1
- Adaptive 500 (`archival`):
  - docs=500, batches=50, questions_top=5, graph_nodes=5, defrag_snapshots=11

Observed runtime behavior:
- Adaptive pipeline remained resilient under occasional 300s repair timeouts; runs continued and completed.
- In this environment/model profile, extraction remained conservative (no emergent nodes/patterns), but v1.1 guarantees non-empty consultation-seeded graph initialization and preserves researcher-selected reading order contracts.

## 2026-03-05 - Adaptive prompt variant framework + A/B/C benchmark harness

### Problem
- Adaptive exploration needed repeatable prompt experimentation to improve question graph quality, but prompts were hardcoded and not run-selectable.
- A/B/C testing lacked a single reproducible benchmark script with graph-level metrics.
- Prompt variants that returned richer question metadata (for example `evidence_blocks`) could break notebook integration because `ResearchQuestion` was constructed from raw question dicts with strict kwargs.

### Solution
- Added `app/historian_agent/adaptive_prompts.py` with centralized run-scoped prompt variants (`v1|v2|v3`) for:
  - batch analysis + system message,
  - why/how enforcement,
  - promotion and change/continuity,
  - seed extraction.
- Added config/runtime plumbing:
  - `TIER0_ADAPTIVE_PROMPT_VARIANT` in `app/config.py`.
  - `prompt_variant` support in `IncrementalCorpusExplorer.explore(...)` and API routes (`app/routes.py`).
  - Prompt variant persisted in `exploration_metadata.prompt_variant`.
- Added adaptive prompt benchmark CLI:
  - `scripts/benchmark_adaptive_prompt_variants.py`
  - Runs variants against `/api/rag/explore_corpus` and writes comparable graph metrics under `logs/prompt_variant_benchmarks/`.
- Added prompt plan doc:
  - `docs/adaptive_prompt_variant_plan.md`
- Added test coverage:
  - `app/tests/test_adaptive_prompts.py`
- Hardened notebook compatibility in `app/historian_agent/research_notebook.py`:
  - sanitize question payload keys before `ResearchQuestion(...)` construction.
  - prevents crashes when prompts include additional diagnostic keys.
- Added direct question-evidence wiring in `app/historian_agent/incremental_explorer.py`:
  - consumes `questions[].evidence_blocks` to add `EvidenceLink`s,
  - optionally mirrors evidence to seed target on decompose-to-seed decisions.

### Verification
Code quality checks:
- `python -m py_compile app/config.py app/routes.py app/historian_agent/adaptive_prompts.py app/historian_agent/corpus_explorer.py app/historian_agent/incremental_explorer.py app/historian_agent/relation_engine.py app/historian_agent/defrag_engine.py app/historian_agent/research_consultation.py app/historian_agent/research_notebook.py scripts/benchmark_adaptive_prompt_variants.py`

Tests:
- `PYTHONPATH=app pytest -q app/tests/test_adaptive_question_graph.py app/tests/test_research_consultation.py app/tests/test_adaptive_prompts.py`
- Result: `12 passed`

Benchmark runs (100 docs, adaptive, archival sort):
- Initial A/B/C (`logs/prompt_variant_benchmarks/adaptive_prompt_benchmark_20260304_204752.json`):
  - `v1`: nodes=11, edges=17, logs=21, promotions=0
  - `v2`: nodes=13, edges=5, logs=10, promotions=0
  - `v3`: nodes=46, edges=34, logs=61, promotions=0
- Post-fix focused `v3` validation (`logs/prompt_variant_benchmarks/adaptive_prompt_benchmark_20260304_210622.json`):
  - `v3`: nodes=50, edges=35, logs=60, promotions=0

Observed behavior:
- `v3` reliably increases graph activity and emergent micro-node generation.
- Promotion bottleneck remains (meso/macro promotion still 0), indicating remaining work is now primarily in promotion/evidence aggregation thresholds rather than prompt output formatting.

## 2026-03-05 - Prompt-quality v4 variant, run-scoped model A/B hooks, and benchmark resiliency

### Problem
- Adaptive prompt experiments needed stronger prompt structure based on current provider best practices.
- Model A/B testing through the API failed when trying to mutate frozen config (`FrozenInstanceError` on `Tier0Config`).
- Benchmark matrix runs were brittle when one model/variant failed and auto-model selection could pick non-text models.

### Solution
- Added adaptive prompt variant `v4` in `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/adaptive_prompts.py` with:
  - ordered instructions,
  - explicit closed-world constraints,
  - quality rubric,
  - compact good/bad examples,
  - preserved JSON output contract.
- Extended prompt tests in `/Users/louishyman/coding/nosql/nosql_reader/app/tests/test_adaptive_prompts.py` to validate `v4`.
- Refactored adaptive run-scoped model selection without mutating frozen config:
  - threaded `ledger_model` through `/Users/louishyman/coding/nosql/nosql_reader/app/routes.py` into explorer calls,
  - added run-scoped model/provider overrides in `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/incremental_explorer.py` and `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/corpus_explorer.py`,
  - threaded explicit `model_name` through relation/defrag calls in `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/relation_engine.py` and `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/defrag_engine.py`.
- Hardened benchmark harness in `/Users/louishyman/coding/nosql/nosql_reader/scripts/benchmark_adaptive_prompt_variants.py`:
  - supports model matrix testing,
  - filters out non-text/cloud/vision models for `--models auto`,
  - continues on per-run errors and records status/error rows,
  - default variants updated to `v2,v3,v4`.
- Added web-review notes and implementation mapping in `/Users/louishyman/coding/nosql/nosql_reader/docs/prompting_best_practices_web_review_2026-03-05.md`.

### Verification
- `python -m py_compile app/historian_agent/adaptive_prompts.py app/historian_agent/relation_engine.py app/historian_agent/defrag_engine.py app/historian_agent/incremental_explorer.py app/historian_agent/corpus_explorer.py app/routes.py scripts/benchmark_adaptive_prompt_variants.py`
- `PYTHONPATH=app pytest -q app/tests/test_adaptive_question_graph.py app/tests/test_research_consultation.py app/tests/test_adaptive_prompts.py` → `13 passed`
- Prompt/model matrix benchmark:
  - `python scripts/benchmark_adaptive_prompt_variants.py --documents 10 --variants v3,v4 --models qwen2.5:32b,gemma3:12b,llama3.1:8b --sort-order archival --timeout 600`
  - output saved to: `/Users/louishyman/coding/nosql/nosql_reader/logs/prompt_variant_benchmarks/adaptive_prompt_benchmark_20260305_001605.json`

## 2026-03-05 - Add historian-process prompt variant v5 and quality proxy metrics

### Problem
- Prompt variants improved structure, but they did not explicitly encode the historian workflow provided by domain guidance:
  observation -> puzzle -> system-level explanation.
- Benchmarks compared graph size/shape, but lacked even lightweight quality proxies for factoid vs analytical question output.

### Solution
- Added adaptive prompt variant `v5` in `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/adaptive_prompts.py` with:
  - explicit process instructions (observation, puzzle, system),
  - five historian question types (causal, institutional, social structure, change-over-time, experience/meaning),
  - stronger rejection of factoid and vague pseudo-analytic prompts.
- Extended prompt tests in `/Users/louishyman/coding/nosql/nosql_reader/app/tests/test_adaptive_prompts.py` to validate `v5` markers.
- Added a framework reference doc: `/Users/louishyman/coding/nosql/nosql_reader/docs/historian_question_generation_framework.md`.
- Extended benchmark reporting in `/Users/louishyman/coding/nosql/nosql_reader/scripts/benchmark_adaptive_prompt_variants.py` with quality proxies:
  - `quality_analytic_ratio`,
  - `quality_factoid_count`,
  - `quality_vague_count`.
- Updated benchmark defaults to include `v5` for new prompt sweeps.

### Verification
- `python -m py_compile app/historian_agent/adaptive_prompts.py scripts/benchmark_adaptive_prompt_variants.py app/tests/test_adaptive_prompts.py`
- `PYTHONPATH=app pytest -q app/tests/test_adaptive_prompts.py app/tests/test_adaptive_question_graph.py app/tests/test_research_consultation.py` -> `14 passed`

## 2026-03-05 - Historian-inductive plan synthesis and v5/v6 prompt architecture update

### Problem
- New historian guidance required a stronger explicit model of how questions emerge from archives:
  - scale jump from local record to system-level explanation,
  - archival vs historical question distinction,
  - four archival signals (repetition/anomaly/change/absence),
  - iterative evidence-first reasoning.
- Existing plan text did not fully map these ideas to concrete implementation tasks and tests.

### Solution
- Updated prompt architecture in `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/adaptive_prompts.py`:
  - expanded `v5` with stronger archive-fit and mechanism constraints,
  - added experimental `v6` prompt family encoding two-level induction and archival->historical transformation,
  - kept output schema stable for backward compatibility.
- Extended prompt tests in `/Users/louishyman/coding/nosql/nosql_reader/app/tests/test_adaptive_prompts.py` for `v6` markers.
- Updated benchmark defaults in `/Users/louishyman/coding/nosql/nosql_reader/scripts/benchmark_adaptive_prompt_variants.py` to include newer variants when desired.
- Rewrote planning artifacts:
  - `/Users/louishyman/coding/nosql/nosql_reader/docs/adaptive_prompt_variant_plan.md` -> synthesized v2 strategy,
  - added `/Users/louishyman/coding/nosql/nosql_reader/docs/adaptive_design_v3.md` with actionable implementation/testing roadmap.

### Verification
- `python -m py_compile app/historian_agent/adaptive_prompts.py scripts/benchmark_adaptive_prompt_variants.py app/tests/test_adaptive_prompts.py`
- `PYTHONPATH=app pytest -q app/tests/test_adaptive_prompts.py` -> `6 passed`
- Relaunched iterative test runs focused on `v4` vs `v5` while keeping qwen3.5 watcher active.

## 2026-03-05 - Benchmark serialization lock to prevent cross-run contamination

### Problem
- Concurrent benchmark processes (manual matrix + qwen watcher) hit the same adaptive endpoint simultaneously and produced mixed/invalid result tables.
- This contaminated A/B/C comparisons and broke methodological consistency.

### Solution
- Added benchmark-level file locking in `/Users/louishyman/coding/nosql/nosql_reader/scripts/benchmark_adaptive_prompt_variants.py`:
  - lock path flag: `--lock-file` (default `/tmp/nosql_adaptive_benchmark.lock`),
  - blocking `fcntl.flock(..., LOCK_EX)` before any API calls,
  - guaranteed unlock in `finally` block.
- This serializes benchmark executions across terminal sessions/watchers and keeps runs comparable.

### Verification
- `python -m py_compile scripts/benchmark_adaptive_prompt_variants.py`
- Clean serialized matrix run completed without model-cross contamination:
  - `/Users/louishyman/coding/nosql/nosql_reader/logs/prompt_variant_benchmarks/adaptive_prompt_benchmark_20260305_014914.json`

## 2026-03-05 - Optional demographic orientation context for adaptive/legacy corpus exploration

### Problem
- The explorer had no explicit corpus-level demographic orientation, so early batch interpretation relied only on local document text and previously accumulated notes.
- We needed a safe way to inject archive orientation context without treating demographics as direct evidence.
- API and benchmark tooling did not support turning this behavior on/off per run, making A/B evaluation difficult.

### Solution
- Added `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/demographic_orientation.py`:
  - reads `/app/data/demographics.db` in read-only mode,
  - summarizes top demographics/occupations/divisions with defensive schema checks,
  - returns `(summary_text, metadata)` for prompt injection and reporting.
- Updated `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/corpus_explorer.py`:
  - new run-scoped fields for orientation text/meta,
  - new `include_demographic_orientation` explore parameter,
  - `_configure_demographic_orientation()` hook,
  - prepends orientation in `_analyze_batch()` as **context-only** with explicit warning text,
  - emits orientation metadata in `exploration_metadata`.
- Updated `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/incremental_explorer.py`:
  - threaded `include_demographic_orientation` through adaptive run path,
  - logs orientation enabled state,
  - includes orientation metadata in adaptive report metadata.
- Updated `/Users/louishyman/coding/nosql/nosql_reader/app/routes.py` to accept and forward `include_demographic_orientation` across explore entry points.
- Updated `/Users/louishyman/coding/nosql/nosql_reader/app/config.py` with:
  - `TIER0_DEMOGRAPHIC_ORIENTATION_DEFAULT` (bool)
  - `TIER0_DEMOGRAPHIC_ORIENTATION_TOP_N` (int)
- Updated `/Users/louishyman/coding/nosql/nosql_reader/scripts/benchmark_adaptive_prompt_variants.py`:
  - added `--demographic-orientation` switch,
  - includes orientation flag in payload and report metadata.
- Added tests in `/Users/louishyman/coding/nosql/nosql_reader/app/tests/test_demographic_orientation.py` for:
  - successful summary generation from temporary sqlite DB,
  - missing DB fallback behavior.

### Verification
- Syntax checks:
  - `python -m py_compile app/historian_agent/incremental_explorer.py app/historian_agent/corpus_explorer.py app/historian_agent/demographic_orientation.py app/routes.py scripts/benchmark_adaptive_prompt_variants.py app/config.py`
- Tests:
  - `PYTHONPATH=app pytest -q app/tests/test_demographic_orientation.py app/tests/test_adaptive_prompts.py app/tests/test_adaptive_question_graph.py app/tests/test_research_consultation.py`
  - Result: `17 passed`
- 100-doc A/B (same model/prompt/sort, orientation OFF vs ON):
  - OFF: `logs/prompt_variant_benchmarks/adaptive_prompt_benchmark_20260305_102319.json`
  - ON: `logs/prompt_variant_benchmarks/adaptive_prompt_benchmark_20260305_111028.json`
  - Observed deltas (ON - OFF):
    - `seed_questions_confirmed +1` (4 vs 3)
    - `graph_edges +2` (30 vs 28)
    - `decision_log_count +2` (33 vs 31)
    - `graph_total_nodes -3` (14 vs 17)
    - `duration_seconds +665.79s` (slower)
  - Interpretation: orientation improved seed anchoring/connection density in this run, but reduced emergent node count and increased latency.

## 2026-03-06 - Corpus Explorer question-graph drill-down UI (macro -> meso -> micro -> documents)

### Problem
- The Corpus Explorer results page showed question lists and summary graph counts, but there was no interactive way to navigate from macro-level questions down to meso/micro questions and then inspect associated source documents.
- Adaptive reports returned `question_graph.tree` as an empty list, so the frontend had no hierarchical data contract for a clean drill-down experience.

### Solution
- Extended adaptive graph export in `/Users/louishyman/coding/nosql/nosql_reader/app/historian_agent/incremental_explorer.py`:
  - added `_build_graph_tree()` to construct a canonical-node hierarchy for UI navigation,
  - uses document-overlap linkage to derive `macro -> meso -> micro` paths when explicit directed edges are sparse,
  - hydrates document metadata via `DocumentStore.hydrate_parent_metadata(...)`,
  - includes per-micro document rows with `doc_id`, `filename`, `block_ids`, and `evidence_types`,
  - populates `question_graph.tree` in `_export_graph_summary()` while preserving existing summary fields.
- Added a dedicated Question Graph Explorer section to `/Users/louishyman/coding/nosql/nosql_reader/app/templates/corpus_explorer.html`:
  - three navigable columns (`Macro`, `Meso`, `Micro`),
  - breadcrumb path display,
  - associated document panel.
- Wired frontend interaction in `/Users/louishyman/coding/nosql/nosql_reader/app/static/corpus_explorer.js`:
  - `renderQuestionGraph(...)` stateful drill-down renderer,
  - level selection and re-render behavior,
  - document evidence panel for selected micro question,
  - graceful hide/fallback when graph tree is absent.
- Added styling for the new explorer in `/Users/louishyman/coding/nosql/nosql_reader/app/static/style.css`:
  - clean multi-column layout on desktop,
  - responsive collapse on smaller screens,
  - selected-state affordances and compact evidence chips.

### Verification
- Syntax:
  - `python -m compileall app/historian_agent/incremental_explorer.py`
  - `node --check app/static/corpus_explorer.js`
- Runtime payload validation (against saved adaptive 1000-doc run):
  - `question_graph.tree` now populated,
  - sample output confirmed `macro_entries=5`, with nested meso/micro items and micro document rows including block IDs.

## 2026-03-07 - Run comparison endpoint, compare UI completion, and graph-aware benchmark rubric

### Problem
- The frontend exposed a new "Compare 2 Runs" control, but there was no backend endpoint to compare two saved exploration artifacts, so the button path failed.
- New compare UI elements lacked dedicated styling, which made notebook selection rows and compare results hard to scan.
- Prompt/model benchmarking still relied mostly on wording heuristics and did not incorporate question-graph quality signals (traceability, duplicate micro rates, macro evidence coverage).

### Solution
- Added notebook compare backend in `/Users/louishyman/coding/nosql/nosql_reader/app/routes.py`:
  - new endpoint: `POST /api/rag/exploration_notebooks/compare`,
  - centralized safe-path resolver `_resolve_saved_notebook_path(...)` to keep compare input constrained to `NOTEBOOK_SAVE_DIR`,
  - JSON loader guard `_load_saved_payload(...)`,
  - compare feature extraction `_extract_report_compare_features(...)` from `question_graph.tree`,
  - shared-theme detection with exact normalized match plus fuzzy fallback,
  - shared macro-doc coverage metrics (`shared_macro_doc_count`, `shared_doc_coverage_ratio`) returned for UI.
- Completed compare and notebook-row styling in `/Users/louishyman/coding/nosql/nosql_reader/app/static/style.css`:
  - added `.ce-notebooks__actions`, `.ce-notebook-item-row`, `.ce-notebook-compare`,
  - added compare panel classes `.ce-compare-grid`, `.ce-compare-columns`, `.ce-compare-column`, `.ce-compare-list`, `.ce-compare-shared`,
  - added responsive collapse behavior for compare columns and notebook rows.
- Refined compare rendering in `/Users/louishyman/coding/nosql/nosql_reader/app/static/corpus_explorer.js`:
  - shared coverage ratio now formatted to fixed precision before display.
- Upgraded benchmark rubric in `/Users/louishyman/coding/nosql/nosql_reader/scripts/benchmark_adaptive_prompt_variants.py`:
  - consumes `question_graph.quality_metrics`,
  - emits graph-quality fields (`quality_traceability_rate`, `quality_duplicate_micro_rate`, `quality_macro_zero_direct_evidence`, `quality_avg_docs_per_macro`),
  - adds composite `quality_rubric_score` with structured `quality_rubric_breakdown`,
  - extends CLI table output to include traceability/duplicate/rubric columns,
  - accepts both `question` and `question_text` keys when scoring top-level questions.

### Verification
- `python -m py_compile app/routes.py app/historian_agent/defrag_engine.py app/historian_agent/incremental_explorer.py app/rag_base.py scripts/benchmark_adaptive_prompt_variants.py`
- `node --check app/static/corpus_explorer.js`
- `PYTHONPATH=app pytest -q app/tests/test_adaptive_question_graph.py app/tests/test_research_consultation.py app/tests/test_adaptive_prompts.py` -> `15 passed`
